import java.util.*;
import java.io.*;
public class get10ref {
	static HashMap<Double, Integer> map = new HashMap<Double, Integer>();

	public get10ref(File dirForReference) throws FileNotFoundException {
		run(dirForReference);
	}
	...
	public static void run(File dir) throws FileNotFoundException {
	...
	public static void writeReference(String input) {
		BufferedWriter bf;
		try{
			bf = new BufferedWriter(new FileWriter("10refMethod/10vReference", true));
			bf.write(input);
			bf.close();
		}
		catch(IOException e){
